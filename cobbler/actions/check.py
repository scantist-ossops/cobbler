        ...
        for item_type in ItemTypes:
            for item in self.api.get_items(item_type.value):
                while "" in item._children: # type: ignore[reportPrivateUsage]
                    ...
                    status.append(f"Item Validity Check: The item {item.name} had an empty string as a child.")
                    item._children.remove("") # type: ignore[reportPrivateUsage]
        if found_invalid_items:
            self.logger.info("[cobbler-check] Items will be reloaded to disk due to invalid children!")
    ...
    def check_selinux(self, status: List[str]) -> None:
        ...
        if enabled:
            status.append(
                ...
                "https://cobbler.rtfd.org/en/latest/user-guide/selinux.html"
            )
